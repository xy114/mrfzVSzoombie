...
5. **打包默认使用安装包（高优先级）**：执行 `npm run build:win` 打包 Windows 版本时默认生成 NSIS 安装包（`.exe` 安装程序）。仅在用户明确要求"便携版"或"运行文件"时才使用 `npm run build:win:portable` 打包便携版。
6. **README 版本号同步**：每次更新 README.md 末尾的版本号时，必须同步更新 `package.json` 中的 `version` 字段，保持两者一致。
